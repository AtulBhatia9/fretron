...
async function main() {
    try {
        ...
        for (const shipment of shipments) {
            try {
                {
                    let mobilenumber = shipment.feetInfo.device.imei
                    let uuid = shipment.uuid
                    const consentData = await consentGet(mobilenumber, uuid);
                    if (consentData.consentStatus === "ALLOWED")
                        if (consentData !== null) {
                            ...
                            await sendDataToSSIl(payload);
                            await bulkSyncApi(payload);
                        } else {
                            console.log("Consent data is null");
                        }
                }
            } catch (error) {
...
async function getShipment() {
    try {
        ...
        console.log(`Total shipments having consent status pending - ${res.length}`)
...
async function consentGet(mobilenumber, uuid) {

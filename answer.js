...
const token =
 "Beaer eyJhbGciOiJIUzI1NiJ9..Es6erVa21gtwNhRI0jQ9NDcOpvNzvapxvCMaujB7oyu";
...
function generateHTML(order, fu) {
 try {
 ...
 order.map((e) => {
 try {
 let orderNo = e.orderNumber;
 let dispatchPlantName =
 e?.salesOffice?.name ?? e?.consignmentBranch?.name ?? "";
 let customerName =
 e?.customer?.name ??
 _.uniq(e.lineItems.map((_) => _.consignee.name)).join() ??
 "";
 let destinations = e?.customer?.places?.[0]?.name ?? "";

 totalWeight +=
 e?.lineItems?.[0]?.mappings?.[0]?.quantity?.weight?.netQuantity ?? 0;

 if (!orderNumber.filter((e) => e === orderNo).length) {
 orderNumber.push(orderNo);
 }
 if (!dispatchPlant.filter((e) => e === dispatchPlantName).length) {
 dispatchPlant.push(dispatchPlantName);
 }
 if (!customer.filter((e) => e === customerName).length) {
 customer.push(customerName);
 }
 if (!Destinations.filter((e) => e === destinations).length) {
 Destinations.push(destinations);
 }
 } catch (error) {
 console.log(`Caught error in mapping order details - ${error.message}`);
 }
 });

 orderNumber = orderNumber.join();
 dispatchPlant = dispatchPlant.join();
 ...
 console.log(`Caught error in generating html file - ${error.message}`);
 }
